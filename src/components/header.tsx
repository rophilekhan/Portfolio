...
import { Link } from "@/lib/type"; // Ensure Link type is correctly defined
...
export default function Header({ links }: HeaderProps) {
    ...
    return (
        ...
        >
            <motion.div className="flex p-1 rounded-none border border-white border-opacity-40 bg-white bg-opacity-80 shadow-lg shadow-black/[0.03] backdrop-blur-[0.5rem] sm:rounded-full dark:bg-gray-950 dark:border-black/40 dark:bg-opacity-75">
                <ul className="flex flex-wrap items-center justify-center gap-y-1 text-[0.9rem] font-medium text-gray-500">
                    {links?.map((link, index) => (
                        <motion.li 
                            className="flex items-center justify-center relative"
                            key={`${link.hash}-${index}`} // Ensure unique key
                        ...
                        >
                            <NextLink
                                className={clsx(
                                    ...
                                    {
                                        "text-red-900 dark:text-purple-700": activeSection === link.hash,
                                    }
                                )}
                                href={link.hash}
                                onClick={() => {
                                    setActiveSection(link.hash);
                                    setTimeOfLastClick(Date.now()); // Call Date.now() correctly
                                }}
                            >
                                {link.nameEng}
